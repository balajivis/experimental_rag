...
export async function loadHierarchicalGeminiMemory(currentWorkingDirectory, includeDirectoriesToReadGemini = [], debugMode, fileService, settings, extensionLoader, folderTrust, memoryImportFormat = 'tree', fileFilteringOptions) {
    ...
    if (debugMode) {
        debugLogger.debug(`CLI: Delegating hierarchical memory load to server for CWD: ${currentWorkingDirectory} (memoryImportFormat: ${memoryImportFormat})`);
    }
    // Directly call the server function with the corrected path.
    return loadServerHierarchicalMemory(effectiveCwd, includeDirectoriesToReadGemini, debugMode, fileService, extensionLoader, folderTrust, memoryImportFormat, fileFilteringOptions, settings.context?.discoveryMaxDirs);
}
/**
...
export async function loadCliConfig(settings, sessionId, argv, cwd = process.cwd()) {
    ...
    const { memoryContent, fileCount, filePaths } = await loadHierarchicalGeminiMemory(cwd, settings.context?.loadMemoryFromIncludeDirectories
        ? includeDirectories
        : [], debugMode, fileService, settings, extensionManager, trustedFolder, memoryImportFormat, memoryFileFiltering);
